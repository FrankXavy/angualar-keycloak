package com.fxf.keycloakbackend.controller;

import com.fxf.keycloakbackend.dto.ResponseMessage;
import com.fxf.keycloakbackend.model.Foo;
...
public class FooController {
    ...
    public ResponseEntity<Foo> detail(@PathVariable("id") int id) {
        Foo foo = foos.stream().filter(f -> f.getId() == id)
    ...
    public ResponseEntity<?> update(@PathVariable("id") int id, @RequestBody Foo foo) {
        Foo fooUpdate = foos.stream().filter(f -> f.getId() == id)
                .findFirst().orElse(null);
